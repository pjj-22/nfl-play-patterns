"""
Analyze fallback statistics for Phase 1 model.
"""
import sys
...
def main():
    print("=" * 70)
    print("PHASE 1 MODEL - FALLBACK ANALYSIS")
    print("=" * 70)
    ...
    # Show sparse groups
    print(f"\n  Sparse groups (<{stats['min_examples_threshold']} examples):")
    sparse_groups = {k: v for k, v in stats['situation_breakdown'].items()
                    if v < stats['min_examples_threshold']}
    for group, count in sorted(sparse_groups.items(), key=lambda x: x[1]):
        print(f"    {str(group):50} {count:>5} plays")
    ...
    sufficient_groups = {k: v for k, v in stats['situation_breakdown'].items()
                        if v >= stats['min_examples_threshold']}
    for group, count in sorted(sufficient_groups.items(), key=lambda x: x[1], reverse=True)[:10]:
        group_str = group.value if hasattr(group, 'value') else str(group)
        print(f"    {group_str:50} {count:>7,} plays")
